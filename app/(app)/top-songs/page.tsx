...
export default async function TopSongsPage({
  ...
  const rows = await getTopEntries(user.id, range, "songs", { search, sort });

  return (
    <div className="space-y-5">
      <h1 className="text-2xl font-semibold tracking-tight">Top Songs</h1>
      <TimeRangeFilter />

      <form className="flex flex-wrap gap-2 rounded-xl border border-zinc-800 bg-zinc-950/70 p-3" method="GET">
        <Input name="search" placeholder="Search songs..." defaultValue={search} className="max-w-xs" />
        <select
          name="sort"
          defaultValue={sort}
          className="h-10 rounded-lg border border-zinc-700 bg-zinc-900 px-3 text-sm text-zinc-100"
          aria-label="Sort top songs"
        >
